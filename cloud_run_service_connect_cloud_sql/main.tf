...
resource "google_cloud_run_service" "default" {
  ...
  template {
	...
	# Set a environment variable for instance connection name
	env {
		name="INSTANCE_CONNECTION_NAME"
		value="INSTANCE_CONNECTION_NAME_SECRET"
	}

	# Set a secret environment variable for database user secret
	env {
		name = "DB_USER"
            	value_from {
              		secret_key_ref {
                		name = google_secret_manager_secret.dbuser.secret_id # secret name
                		key = "latest" # secret version number
              		}
		}
         }
	
	# Set a secret environment variable for database password secret
	env {
		name = "DB_PASS"
            	value_from {
              		secret_key_ref {
	...
	# Set a secret environment variable for database name secret
	env {
		name = "DB_NAME"
            	value_from {
              		secret_key_ref {
  ...
  metadata {
      annotations = {
        "autoscaling.knative.dev/maxScale"      = "1000" # Applied only on revisions, not on initial service creation
	"run.googleapis.com/cloudsql-instances" = "your-project-id:us-central1:your-cloudsql-name" # Applied only on revision, not on initial service creation
